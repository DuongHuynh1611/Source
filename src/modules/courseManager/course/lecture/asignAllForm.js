...
const AsignAllForm = ({ onCancel,courseId, lectureId,setHasError }) => {
    ...
    const asignALl = (values) => {
        executeAsign({
            data:{
                courseId: courseId,
                startDate : values.startDate ? dayjs(values?.startDate).utc().format(DEFAULT_FORMAT) : dayjs(new Date()).format(DEFAULT_FORMAT),
                dueDate : values.dueDate && dayjs(values?.dueDate).utc().format(DEFAULT_FORMAT),
                lectureId: lectureId,
                note : values.note,
            },
            ...
            onError: (err) => {
                if( err.response.data.code == 'ERROR-COURSE-ERROR-0003'){
                    onCancel();
                    showErrorMessage('Chưa có sinh viên hoặc có sinh viên chưa hoàn tất thủ tục, vui lòng kiểm tra lại');
                }
            },
        });
    };

    ...
    const columns = [
        {
            ...
            render: (text, record, index) => {
                ...
                if (record.lectureKind === 1 || isDisabled) {
                    ...
                    return (
                        <Radio checked={lectureid && lectureid === record.id} onChange={() => onSelectChange(record)} />
                    );
                }
            },
        },
        ...
        },
        {
            ...
            render: (text, record, index) => {
                ...
                const isDisabled = checkAsignItem ? checkAsignItem.status : false;
                if (record.lectureKind === 1 && isDisabled) {
                    return <CheckCircleOutlined className={styles.greenCheckIcon} />;
                } else if (isDisabled) {
                    return <CheckCircleOutlined className={styles.greenCheckIcon} />;
                }
            },
            ...
            <ListPage
                ...
                actionBar={
                    <div style={{ float: 'right', margin: '0 0 32px 0' }}>
                        <Button type="primary" disabled={disabledSubmit} onClick={() => setShowPreviewModal(true)}>

...
import { AppConstants, DATE_FORMAT_VALUE, DATE_FORMAT_ZERO_TIME, DEFAULT_FORMAT, TIME_FORMAT_DISPLAY } from '@constants';
...
function RegistrationForm({ formId, actions, dataDetail, onSubmit, setIsChangedFormValues, isEditing, courseId }) {
    ...
    const handleSubmit = (values) => {
        if (dataLocation) {
            ...
            values.student.birthday = formatDateToZeroTime(values?.student.birthday);
        ...
        if(isEditing){
            values.studentId =  values?.studentInfo?.account?.id;
        }
    ...
    useEffect(() => {
        ...
        var studentId = dataDetail?.studentInfo?.account?.id;
        if(isEditing){

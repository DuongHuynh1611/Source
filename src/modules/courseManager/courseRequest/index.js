...
const message = defineMessages({
    objectName: 'Yêu cầu khóa học',
    name: 'Họ và tên',
    course: 'Khóa học',
    status: 'Trạng thái',
    state: 'Tình Trạng',
    registraction: 'Đăng ký',
    phone: 'Số điện thoại',
});
...
const CourseRequestListPage = () => {
    ...
    const columns=[
        ...
        mixinFuncs.renderActionColumn({ 
            // registration: mixinFuncs.hasPermission([apiConfig.registration.create.baseURL,apiConfig.registration.update.baseURL]),
            edit: true, delete: true }, { width: '170px' }),
    ];
    const searchFields = [
        {
            key: 'state',

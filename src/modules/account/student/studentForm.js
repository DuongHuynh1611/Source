...
import { DATE_FORMAT_DISPLAY, DATE_FORMAT_VALUE, DATE_FORMAT_ZERO_TIME, DEFAULT_FORMAT, DEFAULT_FORMAT_ZERO_SECOND } from '@constants/index';
...
const StudentForm = ({ isEditing, formId, actions, dataDetail, onSubmit, setIsChangedFormValues, handleFocus }) => {
    ...
    useEffect(() => {
        dataDetail.birthday = dataDetail?.account?.birthday && dayjs(convertUtcToLocalTime(dataDetail?.account.birthday, DEFAULT_FORMAT, DEFAULT_FORMAT), DEFAULT_FORMAT_ZERO_SECOND);
        setImageUrl(dataDetail.account?.avatar);
        dataDetail.fromDate = dataDetail.fromDate && dayjs(convertUtcToLocalTime(dataDetail.fromDate, DEFAULT_FORMAT, DEFAULT_FORMAT), DEFAULT_FORMAT_ZERO_SECOND);
    ...
    const handleSubmit = (values) => {
        values.birthday = values.birthday && dayjs(values?.birthday).utc().format(DEFAULT_FORMAT);
        values.fromDate = values.fromDate && dayjs(values?.fromDate).utc().format(DEFAULT_FORMAT);
